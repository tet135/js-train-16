...
function compareArrays(arr1, arr2) {
  console.assert(JSON.stringify(arr1.sort()) === JSON.stringify(arr2.sort()), "Масиви не однакові!");
  ...
  if (typeof size === "number") {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < size; j++) {
        row = [...row, Math.floor(Math.random() * 10)];
      }
      matrix = [...matrix, row];
    }
...
function countVowelsAndConsonants(word) {
  ...
  for (const letter of word) {
    vowels.includes(letter.toLowerCase()) ? console.count("Vowels") : console.count("Consonants");
  }
